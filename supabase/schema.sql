...
-- Basar-Verkaufsliste (siehe basar.html/js/basar.js) - eigenständige
-- Funktion neben der Pferdedatenbank, nutzt aber dieselben Konten/denselben
-- Login. Fotos handschriftlicher/gedruckter Verkaufslisten werden per
-- Texterkennung (Tesseract.js, im Browser) ausgelesen und als Artikel
-- angelegt. Gleiches Zugriffsmodell wie horses (geteilte Datenbank für alle
-- eingeloggten Konten). Siehe migration_042_basar.sql für den Verlauf
-- dieser Tabellen bei bereits bestehenden Projekten.
create table if not exists public.basar_artikel (
  ...
  -- Preis bleibt zum Vergleich in "preis" erhalten.
...
-- Verlaufs-Log je Artikel: JEDE Statusänderung/Bearbeitung/Neuanlage landet
-- hier als unveränderlicher Eintrag - Zeitstempel automatisch über
-- "erstellt_at", Nutzername kommt beim Insert aus der eingeloggten Session
-- (siehe js/basar.js).
...
-- Beleg für spätere Kontrolle bei unklarer Texterkennung) - analog zu
-- "horse-images".
insert into storage.buckets (id, name, public)
